...
def alpha_beta(alpha, beta, stare):
    ...
    if stare.j_curent == Joc.JMAX:
        ...
        for mutare in stare.mutari_posibile:
            raspuns=alpha_beta(alpha,beta, mutare)
            if raspuns.scor>scor_curent:
                scor_curent=raspuns.scor
                stare.stare_aleasa = mutare
                if raspuns.scor>alpha:
                    alpha=raspuns.scor
                if alpha>=beta:
                    break
...
def main():
    ...
    tip=input("Dati 1=alphaBeta sau 2=minMax")
